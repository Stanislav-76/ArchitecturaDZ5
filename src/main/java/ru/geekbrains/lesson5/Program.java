...
class Editor3D implements UILayer{
    ...
    public void addModel(){
        ...
        int txCount = new Random().nextInt(3);
        for (int i = 0; i < txCount; i++){
            Texture texture = new Texture();
            model3D.getTextures().add(texture);
        }
        businessLogicalLayer.addModel(model3D);
        System.out.println(model3D + " добавлена");
    }
    ...
    public void removeModel(int i) {
        ...
        int j = 0;
        for (Entity entity: businessLogicalLayer.getAllModels()) {
            if (entity instanceof Model3D && j++ == i) {
                businessLogicalLayer.removeModel((Model3D) entity);
                System.out.println(entity + " удалена");
                break;
            }
        }
